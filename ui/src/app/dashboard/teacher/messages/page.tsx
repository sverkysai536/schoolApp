    ...
    return (
        <div className="p-8 max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-white drop-shadow-md">Teacher Messages</h1>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[600px]">
                <Card glass className="md:col-span-1 p-4 flex flex-col gap-4 border-white/10">
                    ...
                    <div>
                        ...
                        >
                            <option value="" disabled>Select Student/Parent</option>
                            {recipients.map(r => (
                    ...
                    <textarea
                        className="w-full h-32 bg-black/30 border border-white/10 rounded-lg p-3 text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="Type your message..."
                        value={newMessage}
                        onChange={(e) => setNewMessage(e.target.value)}
                    />
                    <Button onClick={sendMessage} disabled={!selectedRecipient || !newMessage} className="w-full">
                        <Send size={16} className="mr-2" /> Send
                    </Button>
                </Card>

                <Card glass className="md:col-span-2 p-0 overflow-hidden flex flex-col border-white/10">
